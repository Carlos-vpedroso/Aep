...
const adicionarAlunoLista = async (req, res) => {
    try {
        ...
        const lista = await ListaFrancaMatutinoViewModel.findOne({
            where: { data: dataAmanha },
        });

        if (!lista) {
            return res.status(404).json({ message: "Nenhuma lista encontrada, final de semana." });
        }

        // Verificar se o aluno já está na lista
        const alunoExistente = await ListaFrancaMatutinoAlunosViewModel.findOne({
...
const removerAlunoLista = async (req, res) => {
    try {
        ...
        // Buscar lista matutina de amanhã
        const lista = await ListaFrancaMatutinoViewModel.findOne({
            where: { data: dataAmanha },
        });
        ...
        const alunoNaLista = await ListaFrancaMatutinoAlunosViewModel.findOne({
            where: { idLista: lista.id, idAluno },
        });
        ...
        await ListaFrancaMatutinoAlunosViewModel.destroy({
            where: { idLista: lista.id, idAluno },
        });
...
const verificarAlunoNaLista = async (req, res, returnData = false) => {
    try {
        ...
        const lista = await ListaFrancaMatutinoViewModel.findOne({ where: { data: dataAmanha } });
        if (!lista) return returnData ? null : res.status(404).json({ message: "Nenhuma lista encontrada, final de semana." });
...
const getAllAlunos = async (req, res) => {
    try {
        ...
        const alunos = await ListaFrancaMatutinoAlunosViewModel.findAll({
