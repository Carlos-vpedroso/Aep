...
async function criarListasViagem() {
  try {
    // 🔹 1. Buscar todas as rotas ativas
    const rotas = await RotasViewModel.findAll({ where: { ativo: true } });
    ...
    for (const rota of rotas) {
      ...
      if (rota.turno === "Noturno") {
        dataViagem = hoje;
      } else if (rota.turno === "Matutino") {
        dataViagem = amanha;
      }
    ...
    console.log("✅ Rotinas de criação de listas finalizadas.");
  } catch (error) {
    console.error("❌ Erro ao criar listas de viagem:", error.message);
  }
}

module.exports = { criarListasViagem };
